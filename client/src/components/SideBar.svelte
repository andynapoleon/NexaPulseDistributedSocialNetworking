<script lang="ts">
  ...
  function updateSidebarClass() {
    ...
    if (window.innerWidth > 1695) { // = 200/0.118
      sidebarClass = "bigSidebar";
    } else if (window.innerWidth > 1017){ //1017
  ...
  <nav>
    <ul>
      {#each items as item}
        <div class="container">
          {#if item.label === "Home"}
            <button class={$homeSelected} on:click={() => onClickHandler(`${item.href}`)}>
          ...
          {:else if item.label === "For You"}
            <button class={$foryouSelected} on:click={() => onClickHandler(`${item.href}`)}>
          ...
          {:else if item.label === "Friends"}
            <button class={$friendsSelected} on:click={() => onClickHandler(`${item.href}`)}>
              <Icon.ProfileCardSolid class="pt-3 w-[2em] h-[2em] text-[#C2C2C2]" />
              {#if sidebarClass === "bigSidebar"}
                <span>{item.label}</span>
              {/if}
            </button>
          {:else if item.label === "Everyone"}
            <button class={$allUsersSelected} on:click={() => onClickHandler(`${item.href}`)}>
          ...
          {:else if item.label === "Requests"}
            <button class={$notificationsSelected} on:click={() => onClickHandler(`${item.href}`)}>
